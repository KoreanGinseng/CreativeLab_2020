...
struct cbGameParam { Vector4 cbProjection; };
...
MofBool CGameApp::Initialize(void){
	...
	gTarget[1].CreateTarget(sw, sh, PIXELFORMAT_R32G32B32A32_FLOAT, BUFFERACCESS_GPUREADWRITE);
	gTarget[2].CreateTarget(sw, sh, PIXELFORMAT_R32G32B32A32_FLOAT, BUFFERACCESS_GPUREADWRITE);
	gTarget[3].CreateTarget(sw, sh, PIXELFORMAT_R32_FLOAT, BUFFERACCESS_GPUREADWRITE);
	return TRUE;
}
/*************************************************************************//*!
		@brief			アプリケーションの更新
...
MofBool CGameApp::Render(void){
	...
	LPRenderTarget ptarget[] = {
		gTarget[0].GetRenderTarget(),
		gTarget[1].GetRenderTarget(),
		gTarget[2].GetRenderTarget(),
		gTarget[3].GetRenderTarget(),
	};
	//作成したテクスチャを描画ターゲットとして設定する
	//深度バッファは元の情報をそのまま使用する
	g_pGraphics->SetMultiRenderTarget(4, ptarget, g_pGraphics->GetDepthTarget());
	//画面のクリア
	...
	//メッシュの描画(深度を出力する)
	CMatrix44 matWorld; gMesh.Render(matWorld, &gShader, &gShaderBind);
	//描画ターゲットを元に戻す
	...
	//確認用に画面右上に深度を表示する
	gTarget[0].Render(CRectangle(0, 0, g_pGraphics->GetTargetWidth() / 4, g_pGraphics->GetTargetHeight() / 4));
	gTarget[1].Render(CRectangle(0, 192, g_pGraphics->GetTargetWidth() / 4, 192 + g_pGraphics->GetTargetHeight() / 4));
	gTarget[2].Render(CRectangle(0, 384, g_pGraphics->GetTargetWidth() / 4, 384 + g_pGraphics->GetTargetHeight() / 4));
	gTarget[3].Render(CRectangle(0, 576, g_pGraphics->GetTargetWidth() / 4, 576 + g_pGraphics->GetTargetHeight() / 4));
